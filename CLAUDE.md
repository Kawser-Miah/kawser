...
- **Slash commands** (`/about`, `/skills`, `/projects`, `/experience`, `/education`, `/achievements`, `/contact`, `/help`, `/clear`) — still canned, from the `COMMANDS` array in `main.js`. These answers were kept verbatim from the reference design and reference fictional project names (`live-bidding-app`, `gps-fleet-tracker`, etc.) that do **not** match `data/projects.json` — a known, intentional exception to the "no fabricated content" rule; don't "fix" them without asking.
