...
A static personal portfolio website — pure HTML, CSS, and vanilla JavaScript with no build tools and no frameworks. All content is loaded from JSON files at runtime via `fetch()`. There is no backend *in this repo*, but two features call an external FastAPI service (`https://api.kawser.me/api/v1`, a separate repo): the contact form (`POST /contact`) and the chat widget's AI mode (`POST /chat`, RAG). `API_BASE` is defined once at the top of `js/main.js`. The API's `CORS_ORIGINS` must include `https://kawser.me` (the deployed origin) or browser calls fail.

The UI is a **terminal / developer-themed** design (mono-font chrome, terminal windows, code-syntax coloring, an orbiting hero visual, a floating AI-assistant chat widget) ported from a React/TypeScript reference design (kept in `Portfolio Redesign Request/`, a separate Vite scaffold — not part of the deployed site, useful only as a visual reference; `npm install && npm run dev` there boots the original design for comparison). The port intentionally reused the reference design's visuals/animations 1:1 wherever possible, but **all content comes from this repo's own `data/*.json` and hardcoded facts** — the reference design's own placeholder content (fictional project names, fake stats) was never carried over. (The chatbot's slash-command answers were originally kept verbatim from the reference design with fictional project names; they have since been rewritten to match this repo's real `data/*.json` and hardcoded facts.)
...
**Chatbot:** the floating terminal AI-assistant widget (`#chatFab`/`#chatPanel`). Two modes:
- **Slash commands** (`/about`, `/skills`, `/projects`, `/experience`, `/education`, `/achievements`, `/contact`, `/help`, `/clear`) — canned, from the `COMMANDS` array in `main.js`. The answers are hand-maintained summaries of this repo's real content (`data/*.json`, the hardcoded `#achievements`, hero stats); keep them in sync when the underlying data changes. `/projects` lists a hand-picked subset by name and points visitors to the full list on the site.
- **AI mode** (toggle top-right) — free-text questions `POST` to `API_BASE + '/chat'` and render the RAG `answer` (Markdown, via the tiny `mdLite()` allow-list renderer: `**bold**`, `` `code` ``, `- ` bullets only — everything HTML-escaped first). Rate limited server-side (3/min per IP → friendly 429 message). On any network/API failure the widget shows a short sys message pointing back to the slash commands — it does **not** fall back to canned AI answers.
...
- **Contact form:** `POST`s JSON `{name, email, message}` to `API_BASE + '/contact'`. Client-side validation mirrors the backend's Pydantic rules (name 2–100, valid email, message 10–5000) so obvious mistakes never leave the page. `#form-status` shows the result: `.is-success` on `202`, `.is-error` on `422` (first `errors[].msg`), `429`, `5xx`, or network failure — the error/failure messages fall back to a `mailto:` link for `kawsermiah.cse@gmail.com`. Submit button is disabled + relabelled "sending…" in flight. Styled as a terminal-style `new_message.dart` form. Note `js/analytics.js` also fires a `contact_submit` Firebase event on every submit attempt — `main.js` deliberately does **not** double-track.
